...
namespace Client_Laia_T1.PR2.APIrest.Pages
{
    public class RegisterModel : PageModel
    {
        ...
        public RegisterModel(ILogger<RegisterModel> logging)
        {
            _logger = logging;
        }
        public void OnGet()
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var response = await client.PostAsJsonAsync("register", Register);

                    if (!response.IsSuccessStatusCode)
                    {
                        _logger.LogInformation("Register failed");
                        ErrorMessage = "Error en alguna credencial o usuari ja registrat.";
                    }
                }
            }
            catch (Exception ex)
